...
| ID   | Item                                | Notes |
|------|-------------------------------------|-------|
| G1   | Gateway four directions + transparent passthrough + audit logging | Split `mode` into `client_api` + `upstream_api` (a2o, o2a, a2a, o2o). Passthrough path via `httputil` semantics (incremental SSE tee, Flush per chunk, X-Accel-Buffering: no). Auth policy: `api_key_env` unset → preserve client headers verbatim (OAuth support). JSONL audit log under `~/.mesh/gateway/<name>/<date>.jsonl` with metadata/full levels, size + age rollover, header redaction, gzip+deflate body decode, SSE event reassembly into `stream_summary` (content, tool_calls, usage, stop_reason, errors). Per-process `run` id disambiguates correlation across restarts. Translation handlers gated through shared `wrapAuditing` for uniform coverage across all four directions. Dashboard renders the gateway section with token counters; Prometheus exposes `mesh_gateway_tokens_in_total` / `_out_total`. Commits `5774514` (config), `da05a4a` (audit), `364ae70` (passthrough), `c831f27` (gzip decode), `7ce9fca` (SSE reassembly), `5796864` (translation audit wrap), `fb35f0c` (dashboard + docs), and follow-ups for duration parser (`Nd`/`Nw`), run id, deflate, token metrics. |
| C3   | gateway   | `thinking` blocks logged-and-dropped instead of silently dropped | OpenAI Chat Completions has no native thinking type; round-trip preservation would be lossy. `slog.Debug` line added in both translation directions for visibility. |
| C4   | gateway   | `response_format` honored via system-prompt injection | `json_object` and `json_schema` (with embedded schema) append a directive to the Anthropic system prompt. Best-effort; documented as imperfect. Unknown types log warn and drop. |
...
| H3   | Concurrent Close audit              | No new findings. Every shared closeable in the runtime is already protected: `onceCloseListener` wraps forwarding listeners, `closeOnce sync.Once` guards SSH shell channels, gateway and filesync HTTP servers use `srv.Shutdown` from a single goroutine, `context.AfterFunc` + deferred close touch only stdlib `net.Conn`/`net.Listener` whose `Close` is idempotent. B4 (agent-fwd double close) was the last live incident. |
| H4   | Goroutine leak audit                | No findings. Every long-running loop in `internal/` has a `case <-ctx.Done(): return` branch: `state.StartEviction`, `clipsync.cleanupPeers` / `refreshHTTPRegistration` / `pollClipboard` / `runUDPServer` / `runUDPBeacon` / `runHTTPServer`, `filesync.scanLoop` / `syncLoop` / `watcher.run`, every `tunnel.runForwardSet*` loop, the gateway `srv.Shutdown` monitor, and the proxy accept loops. Fire-and-forget `go n.postHTTP`/`go n.pullHTTP`/`go n.registerPeerHTTP` paths terminate within their per-request `context.WithTimeout` derived from `n.ctx`. |
| H5   | Timer and ticker audit              | No findings. All production `time.NewTimer` sites in `tunnel`, `proxy`, `clipsync`, `filesync`, and `state` are followed by a `select { case <-ctx.Done(): t.Stop(); ...; case <-t.C: ... }` pattern. All `time.NewTicker` sites have `defer ticker.Stop()`. No `time.After` in production code (only in tests, which is acceptable). |
| H6   | Signal handler audit                | No findings. All three `signal.Notify` call sites (`cmd/mesh/main.go` upCmd and downCmd, `cmd/mesh/sigwinch_unix.go`) have a matching `defer signal.Stop` or returned cleanup closure. |
| H7   | Path traversal audit                | No findings. `filesync.safePath` rejects absolute paths, leading-`..`, NUL, and any join whose `filepath.Abs` result lies outside `folderRoot`. `clipsync.processPayload` and `clipsync.downloadFile` both `filepath.Base` peer-supplied filenames before joining into the staging dir. |
| H8   | Integer overflow audit              | Two findings, both in human-readable size parsers. `clipsync.parseByteSize` (test `62ac8b3`, fix `1f93413`) and `config.ParseBandwidth` (test `7661936`, fix `0d57b1b`) multiplied the parsed integer by the unit multiplier with no overflow check, so values like `9000000000GB` silently wrapped to a negative bytes value and downstream rate limiters / `io.LimitReader` calls treated it as a degenerate bound. Both now reject any value where `n > math.MaxInt64/multiplier`. |
| H9   | Silent error swallowing audit       | No findings. The `_, _ = w.Write(...)` discards in gateway / clipsync / proxy are deliberate write-best-effort patterns where the client may have disconnected. The `_ = f.Close()` defer-discards inside helpers that already returned the read result are correct (read errors propagate; close errors on read-only files are not actionable). Production reads through `io.ReadAll` always have an explicit error check. |
| H10  | Unbounded io reads audit            | No findings. Every `io.ReadAll` of a network body is wrapped in `io.LimitReader` (filesync `maxIndexPayload`, gateway `maxRequestBodySize` via `http.MaxBytesReader`, gziputil `maxSize`), and the SSE scanners use `scanner.Buffer(..., maxSSELineSize)`. `io.Copy` calls into local files use a `LimitReader(resp.Body, maxSyncFileSize)` cap. |
| H11  | Map iteration safety audit          | No findings. Every iteration over a shared map is held under the appropriate `RLock`/`Lock`: clipsync's `n.peers` via `peersMu`, state's `s.components` via `s.mu`, filesync's `n.folders` is set once during `Start` before any reader runs and is only read afterwards. The race detector clean run on the unit suite confirms it. |
| H12  | HTTP server hygiene audit           | One finding: gateway and filesync HTTP servers had only `ReadHeaderTimeout`, leaving the slowloris-style hold open after headers complete. Added `ReadTimeout: 2m` and `IdleTimeout: 60s` to both; deliberately omitted `WriteTimeout` because both servers stream long-lived responses (gateway SSE, filesync file download) controlled by per-request context. clipsync already had the full set. Fix `32b285b`. |
| H13  | SSE parser edge cases               | No actionable findings within scope. The gateway SSE scanner uses `bufio.Scanner` with an explicit `maxSSELineSize` buffer cap. It only handles `data: ` (with space) prefixed lines and ignores `event:`, `id:`, and bare `data:` (no space). OpenAI's actual SSE output uses the spaced form consistently, so spec-strict compliance is not load-bearing today; flagged as a follow-up if a non-OpenAI upstream is added. |
| H14  | Config parser adversarial inputs    | No findings. `FuzzLoadUnvalidated` ran for 20s on Opus 4.6 with 23k execs and surfaced no panics; the existing `FuzzParseTarget` and `FuzzParseByteSize` ran for 10s with ~1.2M execs each and were also clean. The YAML loader rejects integers and strings outside their declared field types; `parseByteSize` and `ParseBandwidth` overflow paths were closed under H8. |
| H15  | Subprocess / command injection      | No findings. Every `exec.Command*` site is documented intentional: `tunnel/exec_unix.go` and `exec_windows.go` run user-configured `password_command` (single-string passed to `sh -c`/`pwsh`/`cmd /C` exactly as configured), `tunnel/shell_unix.go` and `shell_windows.go` launch the user's login shell with no peer-supplied args, `clipsync/clipsync.go` runs OS clipboard tools with fixed argument lists, `cmd/mesh/pid_windows.go` calls `tasklist` with a sanitized integer PID. None mix shell metacharacters with peer-supplied input. |
| H16  | ReDoS audit                         | No findings. `regexp.MustCompile` / `regexp.Compile` are not used anywhere in `internal/`. All matching is done with byte-scanning helpers, `strings.HasPrefix`/`HasSuffix`, or `path.Match` (which has linear complexity). |
| H17  | Log injection audit                 | No actionable findings. Clipsync routes peer-supplied strings through `cleanLogStr` before logging (replacing control chars with `?`). Filesync logs go through `slog`'s text handler which quotes strings containing control characters, mitigating CRLF/ANSI injection in practice. Inconsistency between the two packages flagged as a low-severity hardening note for a future pass; not a real exploit today. |
| H18  | Cryptographic correctness           | No findings. No `md5`, `sha1`, `RC4`, or `DES` references in `internal/`. The codebase uses `crypto/sha256` for file hashing (`hashFile`, `computeBlockSignatures`), `crypto/rand` for nonces and message IDs, and the SSH stack from `golang.org/x/crypto/ssh` for transport. `bytes.Equal` is the only digest comparison and is acceptable here because hashes are public file fingerprints, not authentication tokens. |
| H19  | TLS posture audit                   | No findings. No `tls.Config{InsecureSkipVerify: true}` anywhere. Mesh does not terminate or originate TLS itself — outbound HTTP through gateway and filesync uses Go's default transport with system trust roots. SSH handshakes go through `golang.org/x/crypto/ssh` with explicit host key callbacks (rejected when neither `known_hosts` nor `StrictHostKeyChecking: no` is set, see CLAUDE.md security notes). |
| H20  | Context propagation audit           | No findings. Every outbound network call in `internal/` uses the `*WithContext` constructors: `http.NewRequestWithContext`, `exec.CommandContext`, `dialer.DialContext`. Inbound HTTP handlers receive `r.Context()` and propagate it into upstream gateway calls (`a2o_stream.go` / `o2a_stream.go`). The fire-and-forget `go n.postHTTP` / `go n.pullHTTP` paths derive their per-request context from `n.ctx` so node shutdown cancels them. |
| H21  | Channel send-close correctness      | No findings. Every channel close in `internal/` is on a channel owned by the closer (`dirtyCh` in `folderWatcher.run` is closed only by exit; `notifyCh` in clipsync is `make(..., 1)` and only the owning Node sends; `scanTrigger` in filesync follows the same pattern). No send-after-close path identified. The `closeOnce sync.Once` pattern is in place wherever a closeable could be touched from multiple goroutines. |
| H22  | Atomic read-modify-write audit      | No findings. The only check-then-act pattern (`activeChannels.Load()` + `activeChannels.Add(1)` in `tunnel.handleConn`) is single-threaded — `for newChan := range chans` runs in one goroutine and the spawned handlers only decrement. The `Streams.Add(1)` / `defer Streams.Add(-1)` pairs in BiCopy paths cancel correctly. No multi-writer racy counters. |
| H23  | Lock ordering audit                 | No findings. The two compound-lock paths in clipsync (`registerPeer` holds `peersMu` then `stateMu`, `Broadcast` releases `stateMu` before acquiring `peersMu`) are consistent: `peersMu` is the outer lock and `stateMu` is never held while `peersMu` is taken. Filesync uses per-folder `indexMu` and a node-level `mu` but they are never nested in opposing directions. |
| H24  | HTTP client hygiene audit           | No findings. Every `http.Client` instance in `internal/` has an explicit `Timeout` set (gateway `cfg.UpstreamTimeout` or default, clipsync `2*time.Minute`, filesync `10*time.Minute` with explicit per-request `context.WithTimeout`). All response bodies are closed via `defer func() { _ = resp.Body.Close() }()`. `MaxIdleConnsPerHost` is set on the filesync client; the rest rely on Go's defaults which are reasonable for the small peer counts. |
| H25  | Error wrapping audit                | No findings. No `fmt.Errorf("...: %v", err)` or `: %s` wrapping patterns. Every error chain uses `%w` so `errors.Is`/`errors.As` work end-to-end. Wrapping is consistent: `fmt.Errorf("connections[%d] %q: %w", ...)` is the pattern across config validation. |
| H26  | Retry backoff audit                 | No findings. `tunnel.retryDelay` applies exponential backoff with a configurable cap; the SSH client retry loops use `time.NewTimer(retryDelay(...))` with `ctx.Done` for cancellation. The proxy accept loops use a fixed 50ms backoff after `Accept` errors which is acceptable since `Accept` errors are expected to be transient kqueue/epoll wakes. No unbounded retry budgets. |
| H27  | Typed-nil audit                     | No findings. The `error` interface returns are always either a real error or `nil` directly (no concrete typed-nil pointer wrapped in `error`). Methods that return `*folderState` / `*Component` use a comma-ok pattern when the caller might receive `nil`. |
| H28  | Enum / switch exhaustiveness        | No findings. The `state.Status` set is small (Starting / Listening / Connecting / Connected / Failed / Retrying) and is treated as descriptive metadata, not a state machine — switches over it always have a default branch or only check the specific status they care about. The filesync `Direction` switch in `protocol.go` has explicit cases for `receive-only`, `disabled`, and falls through for `send-receive` / `send-only`, which are the only legal direction values per `validate`. |
| H29  | TOCTOU / atomic write audit         | No findings. `safePath` validates paths once and the file ops happen immediately. `transfer.go` uses `os.Stat` to compute resume offsets followed by `OpenFile` with `O_APPEND` — a race here would require another process writing to the temp file with the same expected-hash name, which only happens if two mesh processes share `~/.mesh/filesync` (unsupported). Atomic writes use `renameReplace` (build-tagged for Windows). |
| H30  | Cross-platform path audit           | No findings. Forward-slash separators flow through `path.Match` (W2 fix) and `filepath.ToSlash` for the network protocol; local FS operations use `filepath.Join` and `filepath.FromSlash`. Windows volume handling goes through `filepath.VolumeName` (W6 fix). Long paths and case sensitivity are not specially handled — Go's stdlib does the right thing on each platform. |
| H31  | Shutdown drain audit                | No findings. The HTTP servers in clipsync, filesync, and gateway all install a shutdown goroutine that calls `srv.Shutdown(shutdownCtx)` with a 5-second context, which lets in-flight requests drain before close. `wg.Wait()` is used in tunnel.runForwardSet and netutil.BiCopy/CountedBiCopy to wait for forwarding goroutines. SSH connections close gracefully via `closeClient sync.Once`. |
...
| PI | SHA-256 as [32]byte in FileEntry | `Hash256` named type with YAML marshaler. Eliminates `hexToBytes`/`bytesToHex` helpers. Protobuf uses `entry.SHA256[:]` directly. All hash params through transfer/retry/download chain changed from `string` to `Hash256`. |
