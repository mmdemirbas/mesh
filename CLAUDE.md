...
**Filesync** — Folder sync with named peer definitions and config-level defaults. Peers are declared as a `peers:` map (name → addresses) at the filesync level; folders reference peer names. A `defaults:` section provides fallback values (peers, direction, ignore patterns) that individual folders can override (peers, direction) or extend (ignore patterns are appended). Config is resolved at load time: `FilesyncCfg.Resolve()` merges defaults, resolves peer names to addresses, and populates `ResolvedFolders []FolderCfg`. Runtime code reads only `ResolvedFolders`. Gzip-compressed protobuf index exchange with delta mode (`since` field skips unchanged entries); `Content-Encoding: gzip` header on both requests and responses. Outgoing index is also delta: `PeerState.LastSentSequence` tracks what was already sent to each peer, so subsequent syncs transmit only new entries. Block-level delta transfer via `POST /delta` — receiver sends SHA-256 block signatures (128 KB blocks), sender returns only changed blocks. Bandwidth throttling via `max_bandwidth` config (token-bucket rate limiter). Dual change detection: fsnotify for real-time + periodic scan as safety net. Direction modes: `send-receive` (default), `send-only`, `receive-only`, `dry-run` (scan + compare without file changes), `disabled` (no activity, visible in dashboard). Conflict resolution uses `.sync-conflict-*` naming with second-granularity timestamps; same-second collisions append a counter suffix to prevent overwriting. Transfer resume via `.mesh-tmp-<hash>-<peerSuffix>` temp files (per-peer isolation via SHA-256 suffix) with offset-based HTTP GET. In-flight path dedup (`claimPath`/`releasePath`) prevents concurrent downloads of the same file. Index persisted as YAML in `~/.mesh/filesync/<folder-id>/`, serialized via per-folder `persistMu` to prevent concurrent goroutine races on the `.tmp` file. Ignore patterns are gitignore-style, configured in YAML only (no `.stignore` files); builtin ignores cover both `.mesh-tmp-*` and `*.mesh-delta-tmp-*`. First-sync tombstone guard: remote tombstones do not delete locally-existing files when `lastSeenSeq=0` (first sync with a peer). Delta transfer validates `FileSize` against `maxSyncFileSize` (4 GB) and caps peer-supplied block hash count to the file's actual block count. Delete handler always writes a tombstone even when the file had no prior index entry, preventing resurrection from third peers. Web UI at `/ui/filesync` on the admin port.

**LLM API Gateway** — HTTP server on a loopback port that forwards requests to an upstream LLM API. The (`client_api`, `upstream_api`) pair selects the direction: cross-API pairs (`anthropic`→`openai`, `openai`→`anthropic`) translate between Anthropic Messages API and OpenAI Chat Completions API; same-API pairs (`anthropic`→`anthropic`, `openai`→`openai`) run as transparent passthrough. `model_map` remaps model names (translation mode only); `api_key_env` (single key) or `api_key_envs` (multi-key with `rotation_policy: round_robin|lru|sticky_session`) references env vars for the upstream key(s). Multi-key pools rotate per-request; passive health (`health.passive.failure_threshold`, default 3 → 30s backoff) and optional active probes (`health.active.enabled` with a payload) degrade misbehaving keys. Routing rules can use `upstream_chain` instead of `upstream_name` for ordered fallback across multiple upstreams; chains advance on 429 / 5xx / network errors. When `api_key_env` is unset, the gateway preserves client auth headers verbatim (required for OAuth-authenticated clients such as Claude Code). Optional `response_model` rebrands the model name in every client-facing response (non-streaming buffered, SSE, passthrough) so two LLM-client sessions hitting different gateway routes display visibly distinct model names. Supports streaming (SSE) with per-event translation, tool use (including parallel tool calls), image content (base64 and URL), and error translation (Anthropic 529 ↔ OpenAI 503 in translation directions; upstream status passed through unchanged in passthrough). Non-loopback bind addresses are rejected at config validation. Request bodies capped at 32 MB, upstream responses at 64 MB, SSE lines at 4 MB. Tool call IDs passed through unchanged. Optional per-gateway audit log (`log:` block) records requests and responses as JSONL to `~/.mesh/gateway/<name>/<date>.jsonl` at level `off`/`metadata`/`full`; rows carry correlation ids, redacted headers (no `Authorization`, `x-api-key`, or `Cookie` ever written), decoded bodies (gzip-aware), `stream_summary` with reassembled content, `timing_ms` (six-segment partition: client_to_mesh / mesh_translation_in / mesh_to_upstream / upstream_processing / mesh_translation_out / mesh_to_client + Other = Total), `attempts[]` with `final_upstream` / `final_key_id` for resilience-path forensics, and `cache_control` markers for prompt-cache visibility. Files roll over by size (`max_file_size`) and are pruned by age (`max_age`). A zero `log:` block is silent; any configured field defaults the level to `metadata`. Upstream rate-limit headers (`anthropic-ratelimit-*` / `x-ratelimit-*`) are captured per-gateway into `~/.mesh/gateway/<name>.quota.json` and exposed via `GET /api/gateway/quota` for external statusline consumers.

**Admin server** — Every `mesh up` starts a local HTTP server on `127.0.0.1:7777`. Port written to `~/.mesh/run/mesh-<node>.port`. UI is a unified SPA at `/ui` with tabs: Dashboard, Clipsync, Filesync, Logs, Metrics, API, Debug. Endpoints: `GET /` (redirects to `/ui`), `GET /ui[/clipsync|/filesync|/logs|/metrics|/api|/debug]` (SPA tabs), `GET /api/state` (JSON state), `GET /api/logs` (recent 1000 log lines from ring buffer), `GET /api/logs/file` (full log file with `?offset=` and `?limit=` params, `X-Log-Size` response header), `GET /api/metrics` (Prometheus text), `GET /api/filesync/folders`, `GET /api/filesync/conflicts`, `GET /api/clipsync/activity`. Gateway-specific endpoints: `GET /api/gateway/audit` (recent rows), `GET /api/gateway/audit/stats` (aggregated counters incl. `content_breakdown` per-section input bytes), `GET /api/gateway/audit/pair`, `GET /api/gateway/sessions/<sid>/events` (live session SSE), `GET /api/gateway/active` and `/api/gateway/active/<id>/events` (in-flight registry), `GET /api/gateway/quota` (upstream rate-limit snapshots). Configure with `admin_addr` in node config; set to `"off"` to disable. Auth failures are tracked in `tunnel.authFailuresByIP` and exposed via `tunnel.SnapshotAuthFailures()` for the metrics endpoint. Process-level metrics: `mesh_process_goroutines`, `mesh_process_open_fds` (Unix only), `mesh_state_components`, `mesh_state_metrics`. A background self-monitor (`selfmon.go`, 30s interval) logs warnings when goroutines, FDs, or state map sizes exceed 10,000.
...
- **Cap unbounded collections** from untrusted input with constants (`maxPeers`, `maxRequestBodySize`, `maxSyncFileSize`, `maxClipboardPayload`, `maxTotalFiles`, `maxTotalPages`).
- **Path traversal protection** — `safePath()` validates relative paths with `filepath.EvalSymlinks` on both root and resolved path, catching symlinks that escape the folder root.
...
**Test builds go to /tmp, never to the repo tree.** For ad-hoc compile or test verification, always `go build -o "$(mktemp -d)/mesh" ./cmd/mesh/`. Do NOT write to `./mesh` (repo root pollution) and do NOT write to `./build/` — the user runs their own binary from `./build/mesh` and overwriting or deleting it disrupts their setup. This also means: do not run `task build` or `task check` casually, because the Taskfile writes to `./build/`; prefer targeted `go build`/`go test` invocations with `-o` pointed at a temp path. If a full `task check` is genuinely required, ask first and do not touch `./build/` afterward.
