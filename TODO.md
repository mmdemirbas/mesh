...
- **Environment variables from client** — Handle `env` session requests. OpenSSH clients send
  `LANG`, `LC_*`, and other vars via `SendEnv`. Currently ignored. Should merge client-sent vars
  into
...
- Consider adding a limit (like max-size etc.) to the clipsync to prevent crashes possibly
  caused by big payloads.



- Should I make powershell or cmd the default windows shell? Powershell is more modern, and becoming
  a standard for windows while cmd is the default behaviour for sshd (I guess).