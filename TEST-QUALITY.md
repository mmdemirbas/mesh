...
| internal/clipsync | 60.3% | All HTTP trust-boundary paths now tested against the real handlers: `/discover` (`41518a7`), and after `df08f4c` extracted the inline closures, `/sync`+`/clip`+`/files` rejection paths closed in `f6eed64`. `serveClip` 100%, `serveDiscover` 92.3%, `serveSync` 79.2%, `serveFiles` 75% | None found in scan | Recent fixes all ship with regression tests: `be3668a` paired with `aef49e1` (Broadcast IPv6 canonicalization); `faf3155` paired with `51fa334` (canReceiveFrom IPv6); `1f93413` and `48ac27a` ship fix+test in the same commit. Older omnibus fixes (`618d567`, `fa8b682`, `af47e93`) predate the fix+test-commit discipline; listener double-close behavior from `618d567` is indirectly pinned by `2a65b4f` and the proxy cancellation tests | LOW |
| internal/tunnel   | 71.6% | In-process SSH runtime harness (`runtime_test.go`) using mesh's own `NewSSHServer` as the peer: `Run`/`NewSSHClient`/`runMultiplex`/`runMultiplexTarget`/`buildSSHConfig`/`runLocalForward` 100% (`df21edf`). `runLocalProxy` 0 → 87.1%, `runRemoteProxy` 0 → 75% via SOCKS+HTTP CONNECT roundtrips in `02be144`. `probeTarget` 56.4 → 74.4% via cache-fallback test; `runForwardSetForTarget` 57.1 → 62.9% via multiplex ExitOnForwardFailure test (`4e38e37`). Other: `connectSSH` 85.7%, `runSession` 83.3%, `handleTCPIPForward` 75.3%, `runRemoteForward` 73%. Earlier bites: `snapshotAuthFailuresIn`/`evictOldAuthFailuresIn` 100% in `b4b6934`; `dialerControlIPQoS` 88.9% in `517489b` | Minor: 3× `strings.Contains(err.Error(), …)` in `runtime_test.go` buildSSHConfig error-message assertions; closing would require new exported sentinels (deferred — same class as the tlsutil minor). All test-side `time.Sleep` calls are inside deadline-bounded polling or retry loops — no sleep-as-synchronization | `df21edf` — harness exposed and fixed a protocol-compliance bug in `handleTCPIPForward`: forwarded-tcpip payload sent `fwdReq.BindPort` (could be 0) instead of the actual ephemeral port, making the client reject channels with "no forward for address" because its forwardList is keyed by actualPort. Regression test: `TestHandleTCPIPForward_PortZeroRoundtrip` | LOW |
| internal/proxy    | 68.5% | Orchestration wrappers closed in `2a65b4f` (`RunStandaloneProxies` 85.7%, `RunStandaloneRelays` 79.6%). Core serving paths already tested | None found in scan | One fix commit (`618d567`, listener double-close sync.Once wrap on Windows) has no inline reproducer, but the behavior is pinned by `2a65b4f`'s orchestration-wrapper tests which exercise context-cancellation-triggered listener close | LOW |
